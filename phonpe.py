...
def main():
    ...
    if selected_page == "Home":
        ...
        df1 = pd.read_csv('C:/Users/prabh/Downloads/Datascience/Project/Phonepe/df/aggregated_transaction.csv')
        transaction_data_page(df1, cursor)
...
def quarter_wise_data_page(cursor):
    ...
    with col2:
        st.sidebar.title("Dash Sunburst Chart")
...
def transaction_data_page(df1, cursor) :
    ...
    sunburst_fig = px.sunburst(data, path=['category'], values='value', title='Total Transaction Values', labels={'category': 'Payment Option'})
